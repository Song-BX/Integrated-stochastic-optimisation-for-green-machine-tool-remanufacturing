"""I/O helpers and Stage 1 gate checks for Stage 3."""
...
def require_stage1_passed(report_path: Path) -> Dict[str, object]:
    ...
    if not report_path.exists():
        raise FileNotFoundError(
            ...
            "Run scripts/scan_data_catalogue.py before Stage 3."
        )
...
def read_stage3_tables(raw_dir: Path) -> Dict[str, pd.DataFrame]:
    missing = [file_name for file_name in REQUIRED_STAGE3_FILES if not (raw_dir / file_name).exists()]
    if missing:
        raise FileNotFoundError(f"Missing Stage 3 input files: {', '.join(missing)}")
    return {
        file_name.removesuffix(".csv"): pd.read_csv(raw_dir / file_name, encoding=CSV_ENCODING)
        for file_name in REQUIRED_STAGE3_FILES
    }
