"""Build Stage 7 instances from Stage 6."""
...
from .structures import Stage7Instance


def build_stage7_instance(tables: Dict[str, pd.DataFrame], config: Stage7Config) -> Stage7Instance:
    """Build a Stage 6 instance and attach Stage 7 metadata."""

    base = build_stage6_instance(tables, _stage6_config(config))
    payload = dict(base.__dict__)
    payload["multiobjective_method"] = config.multiobjective_method
    return Stage7Instance(**payload)


def _stage6_config(config: Stage7Config) -> Stage6Config:
    return Stage6Config(
        ...
        selective_assembly_baseline_rule_id=config.selective_assembly_baseline_rule_id,
        no_selective_assembly_ablation_rule_id=config.no_selective_assembly_ablation_rule_id,
        ...
        time_limit_seconds=config.time_limit_per_solve,
